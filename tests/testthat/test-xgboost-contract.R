# Tests for XGBoost backend contract through the model registry.
# helper-load.R and helper-fixtures.R are auto-sourced by testthat.

test_that("XGBoost backend fits and predicts through the registry", {
  skip_if_not_installed("xgboost")
  skip_if_not("xgboost" %in% unname(sdm_model_choices()))
  ...
  fit <- fit_sdm_model("xgboost", occ, env, background_n = 120, cv_folds = 2, seed = 99, n_cores = 1, max_depth = 3, eta = 0.1, nrounds = 20)
  expect_equal(fit$model_id, "xgboost")
  expect_true(is.list(fit$model))
  expect_true(inherits(fit$model$xgb_fit, "xgb.Booster"))
...
test_that("XGBoost component fit/predict works for ensemble", {
  skip_if_not_installed("xgboost")
  skip_if_not("xgboost" %in% unname(sdm_model_choices()))

  spec <- get_sdm_model("xgboost")
