test_that("ANN backend fits and predicts through the registry", {
  skip_if_not_installed("nnet")
  skip_if_not("ann" %in% sdm_model_ids())
  ...
  fit <- fit_sdm_model("ann", occ, env, background_n = 120, cv_folds = 2, seed = 99, n_cores = 1, size = 3)
  expect_equal(fit$model_id, "ann")
...
test_that("ANN component fit/predict works for ensemble", {
  skip_if_not_installed("nnet")
  skip_if_not("ann" %in% sdm_model_ids())
  spec <- get_sdm_model("ann")
