test_that("CTA backend fits and predicts through the registry", {
  skip_if_not_installed("rpart")
  skip_if_not("cta" %in% sdm_model_ids())
  ...
  fit <- fit_sdm_model("cta", occ, env, background_n = 120, cv_folds = 2, seed = 99, n_cores = 1)
  expect_equal(fit$model_id, "cta")
...
test_that("CTA component fit/predict works for ensemble", {
  skip_if_not_installed("rpart")
  skip_if_not("cta" %in% sdm_model_ids())
  spec <- get_sdm_model("cta")
