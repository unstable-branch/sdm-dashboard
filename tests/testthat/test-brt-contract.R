test_that("BRT backend fits and predicts through the registry", {
  skip_if_not_installed("gbm")
  skip_if_not("brt" %in% sdm_model_ids())
  ...
  fit <- fit_sdm_model("brt", occ, env, background_n = 120, cv_folds = 2, seed = 99, n_cores = 1, n_trees = 50)
  expect_equal(fit$model_id, "brt")
...
test_that("BRT component fit/predict works for ensemble", {
  skip_if_not_installed("gbm")
  skip_if_not("brt" %in% sdm_model_ids())
  spec <- get_sdm_model("brt")
