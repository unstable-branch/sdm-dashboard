test_that("MARS backend fits and predicts through the registry", {
  skip_if_not_installed("earth")
  skip_if_not("mars" %in% sdm_model_ids())
  ...
  fit <- fit_sdm_model("mars", occ, env, background_n = 120, cv_folds = 2, seed = 99, n_cores = 1)
  expect_equal(fit$model_id, "mars")
...
test_that("MARS component fit/predict works for ensemble", {
  skip_if_not_installed("earth")
  skip_if_not("mars" %in% sdm_model_ids())
  spec <- get_sdm_model("mars")
