test_that("FDA backend fits and predicts through the registry", {
  skip_if_not_installed("mda")
  skip_if_not("fda" %in% sdm_model_ids())
  ...
  fit <- fit_sdm_model("fda", occ, env, background_n = 120, cv_folds = 2, seed = 99, n_cores = 1)
  expect_equal(fit$model_id, "fda")
...
test_that("FDA component fit/predict works for ensemble", {
  skip_if_not_installed("mda")
  skip_if_not("fda" %in% sdm_model_ids())
  spec <- get_sdm_model("fda")
