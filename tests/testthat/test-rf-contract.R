# Tests for Random Forest backend contract through the model registry.
...
test_that("RF backend fits and predicts through the registry", {
  ...
  fit <- fit_sdm_model("rf", occ, env, background_n = 120, cv_folds = 2, seed = 99, n_cores = 1, num_trees = 50)
  expect_equal(fit$model_id, "rf")
  expect_true(is.list(fit$model))
  expect_true(inherits(fit$model$ranger_fit, "ranger"))
