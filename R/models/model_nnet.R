# Artificial Neural Network (ANN) SDM backend via the nnet package.

cross_validate_ann <- function(model_data, covariates, size, decay, maxit, rang,
  ...
  fit_fun <- function(i, model_data, fold_id, threshold) {
    ...
    model <- tryCatch({
      ...
      log_message(log_fun, "  ANN CV fold ", i, " failed: ", conditionMessage(e))
      NULL
    })
    ...
    pred <- stats::predict(model, newdata = test_sub, type = "raw")
...
fit_ann_sdm <- function(occ, env_train_scaled, background_n = sdm_default_background_n,
                        ...
                        cv_block_size_km = sdm_default_cv_block_size_km,
                        threshold = sdm_default_threshold,
                        size = 5L, decay = 0.01, maxit = 200L, rang = 0.5,
                        bias_method = "uniform",
                        target_group_occ = NULL,
                        thickening_distance_km = NULL,
                        ...) {
  if (!requireNamespace("nnet", quietly = TRUE)) {
    stop("nnet package is required for ANN fitting but is not installed.", call. = FALSE)
  }
  ...
  if (isTRUE(include_quadratic)) {
    log_message(log_fun, "Note: ANN with hidden layer can capture nonlinearity natively")
  }

  log_message(log_fun, "Fitting ANN SDM with ", nrow(pres_vals), " presences and ",
    nrow(bg_vals), " background points (size=", size, ", decay=", decay, ")")
  ...
  list(
    model = model,
    formula = NULL,
    coefficients = data.frame(Message = "ANN does not produce GLM-style coefficients."),
    ...
    ann_params = list(size = size, decay = decay, maxit = maxit)
  )
}

predict_ann_suitability <- function(fit, env_project_scaled, output_tif, n_cores = 1, log_fun = NULL) {
  if (!is.list(fit) || is.null(fit$model)) stop("fit must be a ANN model fit result list.", call. = FALSE)
