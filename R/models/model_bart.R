# Bayesian Additive Regression Trees (BART) SDM backend via dbarts.

cross_validate_bart <- function(model_data, covariates, ntree, ndpost, nskip,
  ...
  fit_fun <- function(i, model_data, fold_id, threshold) {
    ...
    x_train <- as.matrix(train[, covariates, drop = FALSE])
    y_train <- train$presence
    x_test <- as.matrix(test[, covariates, drop = FALSE])

    model <- tryCatch({
      dbarts::bart(
        x.train = x_train, y.train = y_train,
        ntree = ntree, ndpost = ndpost, nskip = nskip,
        keepevery = 2L, verbose = FALSE,
        nthread = 1L
      )
    }, error = function(e) {
      log_message(log_fun, "  BART CV fold ", i, " failed: ", conditionMessage(e))
      NULL
    })
...
fit_bart_sdm <- function(occ, env_train_scaled, background_n = sdm_default_background_n,
                         ...
                         cv_block_size_km = sdm_default_cv_block_size_km,
                         threshold = sdm_default_threshold,
                         ntree = 200L, ndpost = 1000L, nskip = 500L,
                         bias_method = "uniform",
                         target_group_occ = NULL,
                         thickening_distance_km = NULL,
                         ...) {
  if (!requireNamespace("dbarts", quietly = TRUE)) {
    stop("dbarts package is required for BART fitting. Install with: install.packages('dbarts')", call. = FALSE)
  }
  ...
  if (isTRUE(include_quadratic)) {
    log_message(log_fun, "Note: BART captures nonlinearity natively; quadratic terms not needed")
  }

  log_message(log_fun, "Fitting BART SDM with ", nrow(pres_vals), " presences and ",
    nrow(bg_vals), " background points (", ntree, " trees, ", ndpost, " posterior draws)")
  ...
  importance_df <- NULL
  if (!is.null(varcount) && is.matrix(varcount)) {
    imp <- colMeans(varcount)
    imp_max <- max(imp, na.rm = TRUE)
  ...
  list(
    model = model,
    formula = NULL,
    coefficients = data.frame(Message = "BART does not produce GLM-style coefficients."),
    ...
    bart_params = list(ntree = ntree, ndpost = ndpost, nskip = nskip)
  )
}

predict_bart_suitability <- function(fit, env_project_scaled, output_tif, n_cores = 1, log_fun = NULL) {
  if (!is.list(fit) || is.null(fit$model)) stop("fit must be a BART model fit result list.", call. = FALSE)
