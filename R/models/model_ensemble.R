...
predict_ensemble_glm_rangebag_suitability <- function(fit, env_project_scaled, output_tif, n_cores = 1, log_fun = NULL) {
  ...
  terra::writeRaster(ensemble_suit, output_tif, overwrite = TRUE, wopt = list(gdal = c("COMPRESS=DEFLATE", "PREDICTOR=2", "ZLEVEL=6", "TILED=YES", "NODATA=-9999")))

  disagreement <- abs(glm_suit - rangebag_suit)
  names(disagreement) <- "model_disagreement"
  terra::writeRaster(disagreement, disagreement_tif, overwrite = TRUE, wopt = list(gdal = c("COMPRESS=DEFLATE", "PREDICTOR=2", "ZLEVEL=6", "TILED=YES", "NODATA=-9999")))
