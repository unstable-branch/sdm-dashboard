# Flexible Discriminant Analysis (FDA) SDM backend via the mda package.

cross_validate_fda <- function(model_data, covariates, degree, nprune,
  ...
  fit_fun <- function(i, model_data, fold_id, threshold) {
    ...
    model <- tryCatch({
      ...
      log_message(log_fun, "  FDA CV fold ", i, " failed: ", conditionMessage(e))
      NULL
    })
...
fit_fda_sdm <- function(occ, env_train_scaled, background_n = sdm_default_background_n,
                        ...
                        cv_block_size_km = sdm_default_cv_block_size_km,
                        threshold = sdm_default_threshold,
                        degree = 2L, nprune = NULL,
                        bias_method = "uniform",
                        target_group_occ = NULL,
                        thickening_distance_km = NULL,
                        ...) {
  if (!requireNamespace("mda", quietly = TRUE)) {
    stop("mda package is required for FDA fitting but is not installed.", call. = FALSE)
  }
  ...
  if (isTRUE(include_quadratic)) {
    log_message(log_fun, "Note: FDA using MARS as regression method captures interactions natively")
  }

  log_message(log_fun, "Fitting FDA SDM with ", nrow(pres_vals), " presences and ",
    nrow(bg_vals), " background points (degree=", degree, ")")
  ...
  list(
    model = model,
    formula = NULL,
    coefficients = data.frame(Message = "FDA does not produce GLM-style coefficients."),
    ...
    fda_params = list(degree = degree, nprune = nprune)
  )
}

predict_fda_suitability <- function(fit, env_project_scaled, output_tif, n_cores = 1, log_fun = NULL) {
  if (!is.list(fit) || is.null(fit$model)) stop("fit must be a FDA model fit result list.", call. = FALSE)
