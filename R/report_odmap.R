...
write_odmap_report <- function(result, path_csv, path_md = NULL) {
  ...
  hypotheses <- ""
  model_info <- if (!is.null(result$model_info) && is.list(result$model_info)) result$model_info else list()
  algorithm <- .fmt(.get_config("model_id", model_info$id), "glm")
  algorithm_label <- .fmt(.get_config("model_label", result$model_label %||% model_info$label), "GLM / Logistic regression")

  model_method <- .fmt(result$model_method %||% model_info$method, "Fast presence/background GLM with balanced class weights")
