...
export const modelConfigSchema = z.object({
  ...
  rangebagNBags: z.number().int().min(10).max(1000).default(100).optional(),
  rangebagBagFraction: z.number().min(0.1).max(1).default(0.5).optional(),
  rangebagVarsPerBag: z.number().int().min(1).max(50).default(3).optional(),
  maxnetAutoTune: z.boolean().default(false),
  rfNumTrees: z.number().int().min(100).max(2000).default(500),
  rfMtry: z.number().int().min(1).max(50).optional(),
  rfMinNodeSize: z.number().int().min(1).max(100).default(10),
  gamK: z.number().int().min(3).max(15).default(5),
  xgbMaxDepth: z.number().int().min(3).max(12).default(6),
  xgbEta: z.number().min(0.01).max(0.5).default(0.3),
