...
for contract in ("environment: release", "type=semver,pattern={{version}}", "type=sha,format=long,prefix=sha-", "flavor: latest=false", "provenance: mode=max", "sbom: true", "steps.build.outputs.digest", "image-digests", "Verify images are publicly pullable", "scripts/verify_public_ghcr_images.py image-digests.txt", "scripts/make_release_images_env.py image-digests.txt release-images.env"):
