...
### Climate cache invalidation (Group R)

- **Reg-exp bug fix (the headline issue)**: `plumber/R/helpers/climate_helpers.R::handle_climate_check` used the regex `wc2\.1_<res>m_bio_\d+\.tif$` which required an underscore between `bio` and the digit. Real on-disk files written by `geodata::worldclim_global()` use `wc2.1_<res>m_bio<n>.tif` (NO underscore). The regex never matched, so `available` was always empty and the modern frontend perpetually offered to "re-download" layers that were already on disk. **Group B's CHANGELOG claim of "stop false-positive missing" was misleading** — its only fix was the 502-on-Plumber-down case; this regex bug was never addressed. Fixed by extracting the same three-pattern matching scheme (`_(bio<n>)[^0-9]`, `_(bio<0n>)[^0-9]`, `bio_<n>(^|[^0-9])`) that the legacy `find_worldclim_files` already used correctly.
- **CMIP6 regex fix**: the cmip6 branch used `^bio\d+\.tif$` which matched nothing because `fetch_cmip6_worldclim` writes `wc2.1_<res>m_bioc_<n>.tif`. Same fix.
- **Shared matcher module**: `R/covariates/match_climate_layers.R` (NEW) is the single source of truth for matching. `find_worldclim_files`, `verify_worldclim_cache`, `handle_climate_check`, and `preflight_climate_download` all delegate to it — eliminates the three-way drift that allowed the Plumber helper to fall out of sync with the R pipeline. Returns `list(biovars, files)` so callers can pick by biovar or by path.
- **Pre-spawn short-circuit**: `handle_climate_download` now does a synchronous availability check before spawning `callr::r_bg`. If every requested layer for worldclim/chelsa is already on disk and verified, returns `{job_id, status:"completed", cached:TRUE}` in <50ms. Kills the "downloaded successfully but spent 5–10s on a no-op spawn" UX.
- **Cache manifest with sha256** (P2): `R/covariates/climate_cache_manifest.R` (NEW). After each successful download, `<dir>/.sdm-cache-manifest-v1.json` is written with sha256/size/mtime per file. `handle_climate_check` consults it and trusts the manifest over filename matching (with size+`validate_geotiff` spot-checks) — this is the real "stop invalidating on every change": a corrupt-by-mtime file is detected as missing and forces surgical re-download, while truly unchanged files are skipped without directory listing.
- **Permission / uid-mismatch detection** (P3): `audit_climate_dir_permissions()` walks the three climate dirs at every `/check` request. If a directory contains files the Plumber process cannot read (typical bind-mount uid/gid mismatch), the response includes a `permission_issues` array. The frontend surfaces this as a non-blocking amber banner on the Data tab suggesting `./scripts/dev-start.sh fix-perms`.
- Tests added in `tests/testthat/test-handle-climate-check.R` (13 cases covering geodata/legacy/zero-padded/nested naming, 404.html rejection, resolution filtering, CHELSA, CMIP6).
- **Follow-up fix — runtime sourcing crash (same PR)**: the handler's lazy-load fallback called `source_local()`, a function that does not exist anywhere in the codebase. In the live Plumber runtime the matcher/manifest modules were absent from both module loaders, so the fallback crashed with "could not find function" on every request; the `tryCatch` error handler then used `exists("requested", inherits = FALSE)` — which cannot see the `tryCatch` body's frame — and reported `available:[]` **and** `missing:[]` simultaneously, so the frontend Download button never enabled. Fixes: matcher (`match_climate_layers.R`) and manifest (`climate_cache_manifest.R`) are now registered in `R/engine_load.R` and `R/load.R` (loaded at startup); the unreliable `sys.frame(1)$ofile`-based eager-load block in `covariates_climate.R` is removed (`sys.frame(1)$ofile` evaluates to numeric garbage inside `source()`'s eval frames); all `source_local()` calls replaced with loud `stop()`s; the error handler now reuses a hoisted `requested` and logs via `message()`. 4 new regression tests including a loaders-registration guard.
- **Follow-up fix — Plumber auth gate vs liveness probes**: with `PLUMBER_AUTH_DISABLED=true` the preroute gate required `X-Hono-Internal` on every request, including `/health` and `/ready`, so the Docker healthcheck (`curl localhost:8000/ready`, no headers) failed and Hono's health probe marked Plumber unreachable. The gate now allow-lists `/health` and `/ready` unconditionally (status metadata only, no computation — matches the documented open endpoints). Separately, `PlumberClient._fetch` now defaults to the internal-proxy headers, fixing bare GET reads (`getClimateCheck`, `getConfigDefaults`, `getModels`, `healthCheck`) that previously went out unauthenticated and 401'd.
