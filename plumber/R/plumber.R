...
run_model_background <- function(body, biovars, projection_extent, job_dir, app_dir, job_id) {
  ...
  tryCatch({
    ...
    cfg <- sdm_config(
      ...
      dnn_architecture = body$dnn_multispecies_architecture %||% body$dnnMultispeciesArchitecture %||% "DNN_Medium",
      dnn_multispecies_architecture = body$dnn_multispecies_architecture %||% body$dnnMultispeciesArchitecture %||% "DNN_Medium",
      dnn_multispecies_n_seeds = as.integer(body$dnn_multispecies_n_seeds %||% body$dnnMultispeciesNSeeds %||% 3L),
      ...
      multi_ensemble_models = body$multi_ensemble_models %||% body$multiEnsembleModels,
      multi_ensemble_weighting = body$multi_ensemble_weighting %||% body$multiEnsembleWeighting %||% "auc",
      multi_ensemble_power = as.numeric(body$multi_ensemble_power %||% body$multiEnsemblePower %||% sdm_default_ensemble_power),
      multi_ensemble_min_auc = as.numeric(body$multi_ensemble_min_auc %||% body$multiEnsembleMinAuc %||% sdm_default_ensemble_min_auc),
      multi_ensemble_min_tss = as.numeric(body$multi_ensemble_min_tss %||% body$multiEnsembleMinTss %||% sdm_default_ensemble_min_tss),
      biomod2_models = body$biomod2_models %||% body$biomod2Models,
